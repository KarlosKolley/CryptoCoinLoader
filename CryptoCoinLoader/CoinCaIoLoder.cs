...
namespace CryptoCoinLoader
{
    class CoinCaIoLoder
    {
        ...
        {
            string strDataFile = AppDomain.CurrentDomain.BaseDirectory + "coincapio.dat";
            ...
            string input = "http://coincap.io/front";
            ...
            {
                strLine = "";
                strLine = strDt + DL + strTm + DL + ccurData[i]["long"] + DL + ccurData[i]["short"] + DL;

                strElem = ccurData[i]["price"];
                if (strElem == null) strElem = "0";
                decimal dec = 0;
                if(!Decimal.TryParse(strElem, out dec)) strElem = "0";
                strLine += strElem + DL;

                strElem = ccurData[i]["mktcap"];
                if (strElem == null) strElem = "0";
                else strElem = GetFormattedElem(strElem);
                strLine += strElem + DL;

                strElem = ccurData[i]["supply"];
                if (strElem == null) strElem = "0";
                else strElem = GetFormattedElem(strElem);
                strLine += strElem + DL;

                strElem = ccurData[i]["volume"];
                if (strElem == null) strElem = "0";
                else strElem = GetFormattedElem(strElem);
            ...
            st.WriteLine("***************  COINCAP-IO LOG " + strDt + " at " + strTm + " ***************************");
